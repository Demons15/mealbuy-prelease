...
public class AppForegroundStateManager {
    ...
    private AppForegroundStateManager() {
        // 在主线程创建一个 handler
        mHandler = new NotifyListenersHandler(Looper.getMainLooper(), this);
    }
    ...
    private static class NotifyListenersHandler extends Handler {
        WeakReference<AppForegroundStateManager> appForegroundStateManagerWeakReference;

        private NotifyListenersHandler(Looper looper, AppForegroundStateManager appForegroundStateManager) {
            super(looper);
            appForegroundStateManagerWeakReference = new WeakReference<AppForegroundStateManager>(appForegroundStateManager);
        }

        @Override
        public void handleMessage(Message inputMessage) {
            super.handleMessage(inputMessage);
            AppForegroundStateManager appForegroundStateManager = appForegroundStateManagerWeakReference.get();
            if (appForegroundStateManager != null) {
                switch (inputMessage.what) {
                    // 解码完成
                    case MESSAGE_NOTIFY_LISTENERS:
                        /* 通知所有观察者状态发生了改变 */
                        android.util.Log.v(TAG, "App just changed foreground state to: " + appForegroundStateManager.mAppForegroundState);
